...
\item \talk{Manuela Campanelli (RIT)}{}
...
We evaluate the improvements on contraints on CMB anomalies and dark energy models when observables obtained via the Sunyaev Zel'dovich effect are taken into account an analysis that usually only consideration primary CMB T and E-pol data.
...
\item \talk{Michael Desrochers (University of British Columbia)}{Hamiltonian consistency of the gravitational constraint algebra under deformations}

The importance of the first-class constraint algebra of general relativity is not limited just by its self-contained description of the gauge nature of spacetime, but it also provides conditions to properly evolve the geometry by selecting a gauge.
...
\item \talk{F\'{e}lix-Louis Juli\'{e} (APC, Paris)}{Compact binary systems in modified gravities and EOB theory}

I will show how to derive analytical gravitational waveforms associated to the coalescence of hairy binary black holes, by generalizing the EOB formalism to the example of Einstein-Maxwell-dilaton theories.
...
\item \talk{Chandrima Ganguly (Dartmouth College)}{Eternal Universe: Mixmaster Cosmologies that work}

A non linear equation of state sourcing an anisotropic Mixmaster cosmology is shown to isotropise as well as create a bounce without actively including a negative energy density.
...
\item \talk{Lam Hui (Columbia University)}{}
...
\item \talk{Mark Kasevich (Stanford University)}{}


\item \talk{Justin Khoury (University of Pennsylvania)}{}
...
\item \talk{Rubab Manzoor (University of Management and Technology)}{Self-Gravitating Body and Modified Gravity}
...
\item \talk{Sergey Sushkov (Kazan Federal University)}{}
...
\item \talk{Jonathan Thornburg (Indiana U, Max Planck Gravitational Physics)}{Excitation of Kerr quasinormal modes in extreme--mass-ratio binary black hole inspirals}

If a small "particle" of mass $m$ orbits a "background" black hole of
mass $M >> m$, the leading-order perturbations to the background spacetime
include an $O[(m/M)^2]$ gravitational-radiation-reaction "self-force"
acting locally on the particle.  Using a scalar-field toy model, we
[Phys. Rev. D95, 084043 (2017)] found that that in some cases the
self-force shows large oscillations on the outgoing leg of the orbit
shortly after periapsis passage.  (An antecedent of this result was
found by Kojima and Nakamura [Prog. Theor. Phys. 72, 494 (1984)]: when
an infinitesimal-mass test particle makes a hyperbolic flyby of a Kerr
black hole, in some cases there are small oscillations in the radiated
gravitational waves at ${\cal I}^+$ shortly after the main periapsis-passage
signal.)  Here we extend these results and show that these "wiggles" are
in fact quasinormal-mode oscillations of the background black hole spacetime
excited by the particle's periapsis passage.  Wiggles occur for both the
scalar-field and the full gravitational systems, throughout a wide range
in parameter space.  They are detectable in the local self-force,
strong-field regularized scalar-field or metric perturbations, and in
radiated fields at ${\cal I}^+$.  They represent a situation where
\emph{nonlocal} contributions to the self-force Green function are
important.  Their effects at ${\cal I}^+$ might allow direct observation
of Kerr QNMs, enabling new tests of general relativity.
...
ECOs produce similar ringdown waveforms to the GR black holes, but they are followed by delayed ``echoe''. By solving linearized Einstein equations we can model these echoes.	


\item \talk{Mark Wise (Caltech)}{}
